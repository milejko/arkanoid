...
- Wall layouts follow a repeating 12-level cycle that starts only from level 5: the first cycle level with walls adds `(2,9)`, then subsequent cycle steps add `(7,9)`, `(4,10)`, `(5,10)`, `(2,11)`, `(7,11)`.
...
- Leaderboard reads and writes use a localStorage cache first (`arkanoid-leaderboard-cache`), then synchronize with the Apps Script backend. Frontend/backend changes have to stay in sync across both the cache/payload handling in `script.js` and the server implementation in `google-apps-script/Code.gs`.
