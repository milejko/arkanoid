...
- Resumable round states now persist a gameplay snapshot in `localStorage`: both an active pause and the pre-launch overlay state after a new level or life loss. App boot restores that snapshot before showing the intro leaderboard. If you change pause, startup, round-reset, or runtime state structures, keep the saved-game serializer and restorer in sync.
